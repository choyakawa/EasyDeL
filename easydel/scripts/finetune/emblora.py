...
def main():
    ...
    # 使 embedding 与 lm_head 在 LoRA 训练模式下也参与梯度与更新（非 LoRA 适配）
    # 做法：仅在训练期间将这两个参数的 Param 类型切换为 LoRAParam，使其进入 graphstate 参与反传；
    # 训练结束并合并 LoRA 后再还原回普通 Param，以保证保存权重完整。
    try:
    ...
    # 还原 embedding 与 lm_head 为普通 Param，确保保存到 HF 时权重完整
    try:
